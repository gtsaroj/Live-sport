...
export function MatchInfo({ match }: MatchInfoProps) {
  return (
    <div className="mt-4 sm:mt-6 space-y-4 sm:space-y-6">
          ...
          <div className="flex items-center justify-center w-full">
            <div className="flex flex-1 flex-col items-center">
              <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-background p-1.5 sm:p-2 shadow-md transition-transform hover:scale-110 dark:bg-background">
                <Image
                  src={match.team[0].image || "/placeholder.svg?height=80&width=80"}
              ...
              <h3 className="mt-1.5 sm:mt-2 font-semibold text-sm sm:text-base text-center dark:text-white">
                {match.team[0].name || match.title?.split(" vs ")[0] || "Home Team"}
              </h3>
            </div>

            <div className="flex flex-col items-center justify-center mx-2 sm:mx-8">
              <div className="text-lg sm:text-xl font-bold mb-1 sm:mb-2 dark:text-white">VS</div>
              {/* {match.status === "live" && (
              ...
              {match.status === "upcoming" && (
                <div className="text-xs sm:text-sm font-medium text-muted-foreground dark:text-muted-foreground">{match.matchTime}</div>
              )}
            </div>
            ...
            <div className="flex flex-1 flex-col items-center">
              <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-background p-1.5 sm:p-2 shadow-md transition-transform hover:scale-110 dark:bg-background">
                <Image
                  src={match.team[1].image || "/placeholder.svg?height=80&width=80"}
              ...
              <h3 className="mt-1.5 sm:mt-2 font-semibold text-sm sm:text-base text-center dark:text-white">
                {match.team[1].name || match.title?.split(" vs ")[1] || "Away Team"}
              </h3>
            </div>
          </div>
      ...
      <h2 className="text-lg sm:text-xl font-semibold dark:text-white">Match Details</h2>
      <Separator />

      <div className="space-y-3 sm:space-y-4">
        {match.description && (
          <div>
            <p className="text-sm sm:text-base text-muted-foreground dark:text-muted-foreground">{match.description}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
          {match.venue && (
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <span className="text-sm sm:text-base dark:text-white">{match.venue}</span>
            </div>
          )}
          ...
          {match.matchTime && (
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <span className="text-sm sm:text-base dark:text-white">{match.matchTime}</span>
            </div>
          )}
        </div>
